...
# Create DAG
        ...
        # Add schedule based on execution mode
        if schedule_param is None:
            dag_params["schedule"] = None
        else:
            dag_params["schedule"] = schedule_param
        
        # Add max_active_runs for continuous mode
        if execution_mode == "continuous":
            dag_params["max_active_runs"] = 1
...
# Create task
        {dag_id}_task = KharonOperator(
            ...
            script_path={task_config["script_path"]!r},
            script_id="{dag_id}",
            client_id="{client_id}",
